\ =========================================================================
\  Program Name: example-adc.fs for Mecrisp-Stellaris by Matthias Koch
\
...
\     - An adc task driven by a timer interrupt whose value turns a red
\       led on and off
\
\  Hardware:
...
\     - A third led, red, on PB0 (330 ohm dropping resistor)
\     - A 10 Kohm potentiometer across Gnd and VDD (3.3V) with wiper
\       connected to PB1 (ADC_IN9)
\
\  To run stand-alone:
...
: gpio-init ( -- )
  ...
  \ Enable GPIOB peripheral clock, set pin PB0 as led output (push-pull,
  \ low speed), and set PB1 as analog input (ADC_IN9)
...
\ -------------------------------------------------------------------------
\ ADC driver, input PB1, configured to operate as a simple comparitor
\ A client task registers high-voltage, low-voltage actions (event sinks)
\ by loading the associated word addresses into the pointer variables
\ 'adc1-High-Act and 'adc1-Low-Act, resp.
\ -------------------------------------------------------------------------
12 constant ADC1_COMP_IRQn
3 constant ADC1_COMP_PRIORITY

' null-act variable 'adc-High-Act
' null-act variable 'adc-Low-Act

: adc-init-hw ( -- )
  \ Set up Timer 3 trigger TGRO for ADC1 conversion every 10 ms
  \ Enable timer peripheral clock
  %1 1 lshift RCC_APB1ENR bis!

  \ Configure timer for 10 ms period
  \ 48 Mhz system clock
  \ 48000000/10000 = 4800 prescaler
  \ 10000/100 = 100 period
  %111 4 lshift TIM3_CR1 bic!     \ Edge aligned, up count
  4800 1 - TIM3_PSC h!            \ Prescaler (16 bit register)
  100 1 - TIM3_ARR !							\ Period

	\ Send output trigger when timer updates
	%111 4 lshift TIM3_CR2 bic!
	%010 4 lshift TIM3_CR2 bis!

  \ Set up NVIC for ADC1 global interrupt
  \ Set interrupt priority
  ADC1_COMP_PRIORITY 6 lshift $FF and  \ Priority byte
  ADC1_COMP_IRQn %11 and 3 lshift      \ Bit shift
  lshift
  ADC1_COMP_IRQn 2 rshift 2 lshift     \ Byte offset from NVIC_IPR0
  NVIC_IPR0 + bis!                      \ Write priority bits
  \ Enable interrupt
  %1 ADC1_COMP_IRQn lshift NVIC_ISER bis!

  \ Enable ADC peripheral clock
  %1 9 lshift RCC_APB2ENR bis!
  \ Set up ADC with conversion triggered by Timer 3 Trigger 0
	%1 5 lshift ADC_CFGR1 bic!				\ Align right
	%110111 6 lshift ADC_CFGR1 bic!
	%010011 6 lshift ADC_CFGR1 bis!		\ EXT TRIG, TIM3 TRGO
	%1 13 lshift ADC_CFGR1 bic!				\ Single conversion

	\ One channel, configure ADC_IN9 and sampling time
	%1 9 lshift ADC_CHSELR bis!
	%101 ADC_SMPR bis!		\ 55.5 ADC clock cycles

	\ Start and wait for calibration
	%1 31 lshift %10111 or ADC_CR bic!
	%1 31 lshift ADC_CR bis!
	begin %1 31 lshift ADC_CR @ and while nop repeat

  \ Enable ADC with End of Conversion interrupt trigger
	%1 3 lshift ADC_ISR bis!		\ Clear End of Sequence flag
	%1 3 lshift ADC_IER bis!		\ Enable End of Sequence interrupt
...
: adc-init ( -- )
	['] adc-irq-handler irq-adc !		\ Register adc irq handler
	\ Start ADC triggered conversion
	%1 31 lshift %10111 or ADC_CR bic!
	%1 2 lshift ADC_CR bis!
	%1 TIM3_CR1 bis!					\ Enable TIM3 counter
;
...
\ -------------------------------------------------------------------------
\  Red led task (client of adc1 task)
\  Assumes that led is on if output pin is high.
\ -------------------------------------------------------------------------
%1 constant LED3_MSK

: led3-off ( -- )  LED3_MSK GPIOB_BRR bis! ;

: led3-on ( -- )  LED3_MSK GPIOB_BSRR bis! ;

: led3-init ( -- )
  led3-off
  \ Register with adc
  ['] led3-on 'adc-High-Act !
  ['] led3-off 'adc-Low-Act !
;
\ -------------------------------------------------------------------------
...
: init ( -- )
  ...
  adc-init-hw   \ Initialize adc and driving timer interupt
