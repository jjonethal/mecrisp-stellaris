...
unsigned int read16 ( unsigned int addr )
{
  ...
  //   case 0x40000000:
  //       fprintf(stderr,"IO fetch 0x%08X\n",addr);
  //       data=0xFFFF;
  //       return(data);
...
unsigned int clk_sys_selected = 1;
...
{
   cpsr=0;
   reg_norm[13]=fetch32(0x20000000); // Return stack pointer
   reg_norm[14]=0xFFFFFFFF;          // Link register
   reg_norm[15]=(fetch32(0x20000004) + 2) & ~1; // Reset vector
