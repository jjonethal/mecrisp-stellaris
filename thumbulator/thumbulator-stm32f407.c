...
unsigned int read16 ( unsigned int addr )
{
   ...
   {
     case 0x40004404: // USART2_DR
       data=getchar();
       if (data==127) { data=8; } // Replace DEL with Backspace
       return(data);

     // USART2_SR is not handled, as default value for all memory access into void is $FF: Always be prepared to receive and transmit.
   }
...
{
   if (addr == 0x40023C0C) return(0x00000000); // FLASH_SR -- Never busy.
