...
: 1w.scan ( -- )  
   ...
   BEGIN
    1w.searchrom ( -- flg )
       \ 1W.romid 8 $crc IF CR ." CRC mismatch above" THEN
    dup 1w.romid 7 $crc 1w.romid 7 + c@ =
    	and
    	if
...
\ : t1 0 1w.gettemp if .temp else cr ." Error!" then ;
\ : t2 1 1w.gettemp if .temp else cr ." Error!" then ;
