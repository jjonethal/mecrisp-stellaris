...
//
//   10.11.2014, Matthias Koch:
//   LPC1114FN28 terminal emulation
//
//-------------------------------------------------------------------
...
if(DBUG) fprintf(stderr,"write32(0x%08X,0x%08X)\n",addr,data);
  ...
  // LPC1114FN28 Terminal
  if (addr==0x40008000) { putchar(data&0xFF); fflush(stdout); return; }
...
unsigned int read8 ( unsigned int addr )
{
  ...
  switch (addr)
  {
    case 0x4006A007: // UART Data Register
      data=getchar();
      if (data==127) { data=8; } // Replace DEL with Backspace
      return(data);

    case 0x4006A004: // UART Status Register
      // data=getch();  // Getchar is blocking here.
      // if (data==ERR) { data=0x0080; } else { ungetch(data); data=0x00A0; }
      data=0x00A0; // Always true.
      return(data);

    default: // All other cases handled with general memory access
      data=read16(addr&(~1));
      if(addr&1) { data>>=8; }
      return(data&0xFF);
  }
...
void write8 ( unsigned int addr, unsigned int data )
{
  ...
  switch (addr)
  {
    // Flash controller emulation:
    case 0x40020000: // Flash FTFA_FSTAT register
      if (data=0x80) { write32(flash_addr, flash_data); }
      break;

    case 0x40020004: flash_addr &= 0x00FFFF00; flash_addr |=  data & 0xFF;      break;
    case 0x40020005: flash_addr &= 0x00FF00FF; flash_addr |= (data & 0xFF)<<8;  break;
    case 0x40020006: flash_addr &= 0x0000FFFF; flash_addr |= (data & 0xFF)<<16; break;

    case 0x40020008: flash_data &= 0xFFFFFF00; flash_data |=  data & 0xFF;      break;
    case 0x40020009: flash_data &= 0xFFFF00FF; flash_data |= (data & 0xFF)<<8;  break;
    case 0x4002000A: flash_data &= 0xFF00FFFF; flash_data |= (data & 0xFF)<<16; break;
    case 0x4002000B: flash_data &= 0x00FFFFFF; flash_data |= (data & 0xFF)<<24; break;
...
if(DBUG) fprintf(stderr,"read32(0x%08X)=",addr);
  ...
  // LPC1114FN28 Terminal
  if (addr==0x40008000) { data=getchar(); if (data==127) { data=8; } return(data); } // Replace DEL with Backspace 
  if (addr==0x40008014) { return(0xFF); } // Terminal always available
