...
@ Schreiben und Löschen des Flash-Speichers im STM32L073RZ.
@ Write and Erase Flash in STM32L073RZ.
...
.equ FLASH_PECR,    FLASH_Base + 0x04 @ Flash Program and erase control register
.equ FLASH_PDKEYR,  FLASH_Base + 0x08 @ Flash Power-down key register
.equ FLASH_PEKEYR,  FLASH_Base + 0x0C @ Flash PECR unlock key register
.equ FLASH_PRGKEYR, FLASH_Base + 0x10 @ Flash Program and erase key register
.equ FLASH_OPTKEYR, FLASH_Base + 0x14 @ Flash Option bytes unlock key register
.equ FLASH_SR,      FLASH_Base + 0x18 @ Flash Status register
.equ FLASH_OBR,     FLASH_Base + 0x1C @ Flash Option bytes register
.equ FLASH_WRPROT,  FLASH_Base + 0x20 @ Flash Write protection register
...
@ -----------------------------------------------------------------------------
  ...
  @ Flash memory is mirrored, use true physical address for write
  ldr r2, =0x08000000
  adds r0, r2
...
@ -----------------------------------------------------------------------------
  ...
  @ Löscht einen 128 Bytes großen Flashblock  Deletes one 128 Bytes Flash page
flashpageerase:
@ -----------------------------------------------------------------------------
  push {r0, r1, r2, r3, lr}
  ...
  ldr r2, =FLASH_PECR
  ldr r3, =BIT9+BIT3  @ ERASE + PROG
  str r3, [r2]

  @ Select page to erase:

  movs r1, #0x7F
  bics r0, r1 @ Write 0 to first word of Program page - mask away lower bits. One page is 32 words = 4*32 Bytes = 128 Bytes long.
  movs r2, #0
  str r2, [r0]
